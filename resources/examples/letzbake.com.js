...
async function getBlockCommission(index) {
    ...
    const results = { count: 0, fee: 0, index, baker: block.metadata.baker }

    block.operations.map(op => op.map(transaction => {
...
(async () => {
    ...
    console.log('Bakers fees total', total.fee)
    ...
    console.table(results.map(v => ({ baker: v.baker, block: v.index, txCount: v.count, fee: v.fee })))
})()
